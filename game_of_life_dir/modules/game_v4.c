...
game* new_game(int rows, int cols) {
	...
	char** game_board = (char**)malloc(rows*sizeof(char*) + rows*(cols+1)*sizeof(char)); //Allocate each pointer to a row, 
															//AND each row contiguous in memory
															//Note the extra char for \0 at the end of each row

	int i;
	char* flat_matrix = (char*)game_board;  
	for ( i = 0; i < rows; i++) {//Map each pointer to its row. 
		game_board[i] = (flat_matrix + (rows*sizeof(char*)/sizeof(char) + i*(cols+1))); //Compute the memory location of row i contiguous in memory allocated by malloc
		memset(game_board[i], DEAD, (cols)*sizeof(char)); //should still be valid way to set memory 
...
void free_game(game* g) {
	...
	free(g->board); //Malloc'ed this in one shot, so use corresponding free command 
